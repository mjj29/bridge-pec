...
majority of this system is allowed at Level 4 \orange{9B}, which is the
standard tournament level in England. The major suit openings are only allowed
at Level 5 \orange{9F} and hence alternatives are provided for Level 4 events.
Events not directly run by the EBU may have other restrictions.  References to
the Orange Book sections allowing each convention at Level 4 or 5 are given
...
extended rule of 25 in all cases. \orange{10B4}
...
16+, not game forcing \orange{11C12}. If balanced or semi-balanced then 16--19. If a two suiter (5422 or longer) or three suiter (4441 / 5440) then 16--GF. If a single suiter (6332 or longer) then 16--GF. (16--GF here is 'less than a 3-loser hand').

\paragraph{1$\heartsuit$ (Level 5)}

Varies depending upon position and vulnerability, but always 10--15 points, not
balanced and promises 4 spades \orange{11R}.
...
\paragraph{1$\heartsuit$ (Level 4)}

Varies depending upon position and vulnerability, but always 10--15 points, not
balanced and promises 4 hearts \orange{11C15}.

First, second or third seat non-vulnerable (events with 7+ boards per round only):
\begin{itemize}
\item 4+ hearts, may be longer a minor (possible canape), but will always have 5 hearts or 4 spades.
\end{itemize}

All other positions and vulnerabilities:
\begin{itemize}
\item 5+ hearts, may contain a longer minor (possible canape).
\end{itemize}


\paragraph{1$\spadesuit$ (Level 5)}

10--15 points, not balanced. 5+ hearts, 0-3 spades \orange{11R}, may have a longer minor (possible canape).


\paragraph{1$\spadesuit$ (Level 4)}

10--15 points, not balanced. 5+ spades \orange{11C15}, may have a longer minor (possible canape).

\paragraph{1NT}

Varies depending upon position, vulnerability and number of boards per round \orange{10A8}
   ...
   \item 1$\spadesuit$ = 8+, game-forcing.
      ...
      \item simple rebids are suction-style, showing either the suit above
		...
		\item Could be 3-suited, short in the suit bid. These are shown: 
         ...
         \item 2$\spadesuit$-2NT-3$\spadesuit$: short spades.
			\end{itemize}
		\item Continuations:
			\begin{itemize}
			...
			\item After GF+ suit agreement with a 3-suiter if Swiss is still available then 
				4$\clubsuit$/$\diamondsuit$ is Swiss \xref{sec:swiss}, otherwise suit bids are
...
\item 1$\heartsuit$ (if denying $\heartsuit$, Level 5):
	\begin{itemize}
\label{note:4}
...
\item 1$\heartsuit$ (if not denying $\heartsuit$, Level 5):
	\begin{itemize}
...
\item 1$\heartsuit$ (Level 4):
	\begin{itemize}
	\item 1$\spadesuit$ = forcing enquiry.
		\begin{itemize}
      \item 1NT = both majors. Continuations as after artificial 1NT opening \xref{sec:resp:1nart}.
      \item 2$\clubsuit$ = 5+ hearts and 4+ clubs
      \item 2$\diamondsuit$ = 5+ hearts and 4+ diamonds
      \item 2$\heartsuit$ = 6+ hearts, no outside suit
		\end{itemize}
   \item 1NT = 6-9 no better bid.
   \item 2$\heartsuit$/3$\heartsuit$/4$\heartsuit$ = to play/limit raise/to play.
   \item 2$\clubsuit$/$\diamondsuit$/$\heartsuit$ = natural, forcing.
   \item 2NT = good raise
		\begin{itemize}
		\item 3x = long suit trial
		\item 3$\heartsuit$ = minimum
		\item 3$\spadesuit$/4$\clubsuit$/$\diamondsuit$ = cue, slam try
		\item 4$\heartsuit$ = extras, no cue or trial
		\end{itemize}
   \item 2$\spadesuit$/3$\clubsuit$/3$\diamondsuit$ = fit jump.
   \item 3$\spadesuit$/4$\clubsuit$/$\diamondsuit$/ = splinter
	\end{itemize}


\item 1$\spadesuit$ (Level 5):
	\begin{itemize}
   \item 2$\heartsuit$/3$\heartsuit$/4$\heartsuit$ = to play/limit raise/to play.
   \item 1NT = 1-round force.
   \item 2$\clubsuit$/$\diamondsuit$ = natural, forcing.
...
\item 1$\spadesuit$ (Level 4):
   ...
   \item 2NT = good raise
		\begin{itemize}
		\item 3x = long suit trial
		\item 3$\spadesuit$ = minimum
		\item 4$\clubsuit$/$\diamondsuit$/$\diamondsuit$ = cue, slam try
		\item 4$\spadesuit$ = extras, no cue or trial
		\end{itemize}
   \item 3$\clubsuit$/3$\diamondsuit$/$\heartsuit$ = fit jump.
   \item 4$\clubsuit$/$\diamondsuit$/$\heartsuit$ = splinter
	\end{itemize}
...
\item 2$\diamondsuit$:
   ...
   \item 2NT = F1, how good are your hearts?
      ...
      \item 3$\heartsuit$ = good, single-suited, points spread.
      \item 3$\spadesuit$ = GF opening.
...
This refers only to a natural 1NT, though it may be 9--11, 11--13, 12--15,
14--16, or 16--19 depending on position, vulnerability and auction so far.
Maximum is an 11-count, good 12--13-count, 14--15-count, good 14--15-count or
18--19-count depending on range.

After a 9--15 no trump, or a NV 12--15 no trump (NV 3rd and 4th respectively),
or after 1$\clubsuit$--1$\diamondsuit$--1NT, all bids are natural and to play.
...
\item 4$\clubsuit$ is Roman Key-Quant Gerber \xref{sec:rkqg}.
...
After a NT opening or rebid, 4$\clubsuit$ is Roman Key-Quant Gerber, asking for
aces and quantatitive. Responses are:

\begin{itemize}
	\item 4$\diamondsuit$ = 0 or 3 aces
	\item 4$\heartsuit$ = 1 or 4 aces
	\item 4$\spadesuit$ = 2 aces and denying a quant raise
	\item 4NT = 2 aces and accepting the quant raise
\end{itemize}

After 4$\diamondsuit$/$\heartsuit$, next step asks for quant, with 4NT denying
...
Double is standard takeout (or a strong hand)~\orange{11N3}. After a double and
...
Jump cuebids are stopper-asking for 3NT \orange{11N18}; they promise a long
...
(over a minor) \orange{11N7}. Again, it's weak or strong.
...
top tricks, and 2N shows a non-touching two suiter. \orange{11P7}
...
1$\diamondsuit$, which is now forcing, jump rebids show GF single suited hands.

\subsection{Two level openings}

\begin{itemize}
\item 2$\clubsuit$ = 5-10HCP, 4+ hearts, 4+ spades
	\begin{itemize}
	\item 2$\diamondsuit$ = enquiry as to strength and better major
	\item either major at any level = to play
	\end{itemize}
\item 2$\diamondsuit$ = One of: 6+ weak major OR 21-24 bal OR 2-suited GF not $\spadesuit$+$\diamondsuit$
	\begin{itemize}
	\item 2$\heartsuit$ = pass or correct
		\begin{itemize}
		\item pass = weak hearts
		\item 2$\spadesuit$ = weak spades
		\item 2NT = balanced
		\item 3$\clubsuit$ = GF minors
		\item 3$\diamondsuit$ = GF reds
		\item 3$\heartsuit$ = GF majors
		\item 3$\spadesuit$ = GF blacks
		\item 3NT = GF rounds
		\end{itemize}
	\item 2NT = enquiry
		\begin{itemize}
		\item 3$\clubsuit$ = bad weak hearts
		\item 3$\diamondsuit$ = bad weak spades
		\item 3$\heartsuit$ = good weak hearts
		\item 3$\spadesuit$ = good weak spades
		\item 3NT = balanced
		\item 4$\clubsuit$ = GF minors
		\item 4$\diamondsuit$ = GF reds
		\item 4$\heartsuit$ = GF majors
		\item 4$\spadesuit$ = GF blacks
		\item 4NT = GF rounds
		\end{itemize}
	\end{itemize}
\item 2$\heartsuit$ = 5-10HCP, 5+ hearts, 4+ minor
	\begin{itemize}
	\item 2NT = enquiry for minor and strength
	\item either minor at any level = paradox 
	\end{itemize}
\item 2$\spadesuit$ = 5-10HCP, 5+ hearts, 4+ minor
	\begin{itemize}
	\item 2NT = enquiry for minor and strength
	\item either minor at any level = paradox 
	\end{itemize}
